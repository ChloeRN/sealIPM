...
sam.modA <- as.matrix(readRDS('220718_IPMtest_eHAD_iceSim.rds'))
sam.modB <- as.matrix(readRDS('220718_IPMtest_fSAD_iceSim.rds'))
sam.modC <- as.matrix(readRDS('220718_IPMtest_fSAD&eHAD_iceSim.rds'))
...
data.modA <- melt(sam.modA[, params])
data.modB <- melt(sam.modB[, params])
data.modC <- melt(sam.modC[, params])

## Add information on harvest scenario
data.modA$Type <- 'eHAD'
data.modB$Type <- 'fSAD'
data.modC$Type <- 'fSAD & eHAD'
...
data.comb <- rbind(data.modA, data.modB, data.modC)
colnames(data.comb) <- c('Sample', 'Parameter', 'Estimate', 'Type', 'Environment')
...
pdf('220719_ModelComparison_Types.pdf', width = 11, height = 8)

# Survival parameters
ggplot(subset(data.comb, Parameter%in%Surv.params), aes(x = Estimate)) + 
  geom_density(aes(color = Type, fill = Type), alpha = 0.5) + 
...
############################################################
# PLOTTING ESTIMATES ACROSS TIME FOR DIFFERENT MODEL TYPES #
############################################################
...
sim.Tmin <- 22
sim.Tmax <- 70
ModelType <- c('eHAD', 'fSAD', 'fSAD & eHAD')
ann.est <- data.frame()
...
## Plot comparisons to pdf
pdf('220718_ModelComparisonTime_Types.pdf', width = 11, height = 6)
ggplot(ann.est) + 
